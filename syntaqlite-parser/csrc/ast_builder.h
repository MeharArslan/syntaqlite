/*
** The author disclaims copyright to this source code.  In place of
** a legal notice, here is a blessing:
**
**    May you do good and not evil.
**    May you find forgiveness for yourself and forgive others.
**    May you share freely, never taking more than you give.
**
** @generated by syntaqlite-codegen — DO NOT EDIT
*/

#ifndef SYNQ_AST_BUILDER_H
#define SYNQ_AST_BUILDER_H

#include "csrc/ast.h"
#include "syntaqlite/ast_nodes.h"

#ifdef __cplusplus
extern "C" {
#endif

// ============ Builder Functions ============

static inline uint32_t synq_ast_aggregate_function_call(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan func_name,
    SyntaqliteAggregateFunctionCallFlags flags,
    uint32_t args,
    uint32_t orderby,
    uint32_t filter_clause,
    uint32_t over_clause
) {
    return synq_ast_build(ctx,
        &(SyntaqliteAggregateFunctionCall){
            .tag = SYNTAQLITE_NODE_AGGREGATE_FUNCTION_CALL,
            .func_name = func_name,
            .flags = flags,
            .args = args,
            .orderby = orderby,
            .filter_clause = filter_clause,
            .over_clause = over_clause
        }, (uint32_t)sizeof(SyntaqliteAggregateFunctionCall));
}

static inline uint32_t synq_ast_cast_expr(
    SynqAstContext *ctx,
    uint32_t expr,
    SyntaqliteSourceSpan type_name
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCastExpr){
            .tag = SYNTAQLITE_NODE_CAST_EXPR,
            .expr = expr,
            .type_name = type_name
        }, (uint32_t)sizeof(SyntaqliteCastExpr));
}

static inline uint32_t synq_ast_column_ref(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan column,
    SyntaqliteSourceSpan table,
    SyntaqliteSourceSpan schema
) {
    return synq_ast_build(ctx,
        &(SyntaqliteColumnRef){
            .tag = SYNTAQLITE_NODE_COLUMN_REF,
            .column = column,
            .table = table,
            .schema = schema
        }, (uint32_t)sizeof(SyntaqliteColumnRef));
}

static inline uint32_t synq_ast_compound_select(
    SynqAstContext *ctx,
    SyntaqliteCompoundOp op,
    uint32_t left,
    uint32_t right
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCompoundSelect){
            .tag = SYNTAQLITE_NODE_COMPOUND_SELECT,
            .op = op,
            .left = left,
            .right = right
        }, (uint32_t)sizeof(SyntaqliteCompoundSelect));
}

static inline uint32_t synq_ast_subquery_expr(
    SynqAstContext *ctx,
    uint32_t select
) {
    return synq_ast_build(ctx,
        &(SyntaqliteSubqueryExpr){
            .tag = SYNTAQLITE_NODE_SUBQUERY_EXPR,
            .select = select
        }, (uint32_t)sizeof(SyntaqliteSubqueryExpr));
}

static inline uint32_t synq_ast_exists_expr(
    SynqAstContext *ctx,
    uint32_t select
) {
    return synq_ast_build(ctx,
        &(SyntaqliteExistsExpr){
            .tag = SYNTAQLITE_NODE_EXISTS_EXPR,
            .select = select
        }, (uint32_t)sizeof(SyntaqliteExistsExpr));
}

static inline uint32_t synq_ast_in_expr(
    SynqAstContext *ctx,
    SyntaqliteBool negated,
    uint32_t operand,
    uint32_t source
) {
    return synq_ast_build(ctx,
        &(SyntaqliteInExpr){
            .tag = SYNTAQLITE_NODE_IN_EXPR,
            .negated = negated,
            .operand = operand,
            .source = source
        }, (uint32_t)sizeof(SyntaqliteInExpr));
}

static inline uint32_t synq_ast_is_expr(
    SynqAstContext *ctx,
    SyntaqliteIsOp op,
    uint32_t left,
    uint32_t right
) {
    return synq_ast_build(ctx,
        &(SyntaqliteIsExpr){
            .tag = SYNTAQLITE_NODE_IS_EXPR,
            .op = op,
            .left = left,
            .right = right
        }, (uint32_t)sizeof(SyntaqliteIsExpr));
}

static inline uint32_t synq_ast_between_expr(
    SynqAstContext *ctx,
    SyntaqliteBool negated,
    uint32_t operand,
    uint32_t low,
    uint32_t high
) {
    return synq_ast_build(ctx,
        &(SyntaqliteBetweenExpr){
            .tag = SYNTAQLITE_NODE_BETWEEN_EXPR,
            .negated = negated,
            .operand = operand,
            .low = low,
            .high = high
        }, (uint32_t)sizeof(SyntaqliteBetweenExpr));
}

static inline uint32_t synq_ast_like_expr(
    SynqAstContext *ctx,
    SyntaqliteBool negated,
    uint32_t operand,
    uint32_t pattern,
    uint32_t escape
) {
    return synq_ast_build(ctx,
        &(SyntaqliteLikeExpr){
            .tag = SYNTAQLITE_NODE_LIKE_EXPR,
            .negated = negated,
            .operand = operand,
            .pattern = pattern,
            .escape = escape
        }, (uint32_t)sizeof(SyntaqliteLikeExpr));
}

static inline uint32_t synq_ast_case_expr(
    SynqAstContext *ctx,
    uint32_t operand,
    uint32_t else_expr,
    uint32_t whens
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCaseExpr){
            .tag = SYNTAQLITE_NODE_CASE_EXPR,
            .operand = operand,
            .else_expr = else_expr,
            .whens = whens
        }, (uint32_t)sizeof(SyntaqliteCaseExpr));
}

static inline uint32_t synq_ast_case_when(
    SynqAstContext *ctx,
    uint32_t when_expr,
    uint32_t then_expr
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCaseWhen){
            .tag = SYNTAQLITE_NODE_CASE_WHEN,
            .when_expr = when_expr,
            .then_expr = then_expr
        }, (uint32_t)sizeof(SyntaqliteCaseWhen));
}

static inline uint32_t synq_ast_case_when_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_CASE_WHEN_LIST, list_id, child);
}

static inline uint32_t synq_ast_foreign_key_clause(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan ref_table,
    uint32_t ref_columns,
    SyntaqliteForeignKeyAction on_delete,
    SyntaqliteForeignKeyAction on_update,
    SyntaqliteBool is_deferred
) {
    return synq_ast_build(ctx,
        &(SyntaqliteForeignKeyClause){
            .tag = SYNTAQLITE_NODE_FOREIGN_KEY_CLAUSE,
            .ref_table = ref_table,
            .ref_columns = ref_columns,
            .on_delete = on_delete,
            .on_update = on_update,
            .is_deferred = is_deferred
        }, (uint32_t)sizeof(SyntaqliteForeignKeyClause));
}

static inline uint32_t synq_ast_column_constraint(
    SynqAstContext *ctx,
    SyntaqliteColumnConstraintKind kind,
    SyntaqliteSourceSpan constraint_name,
    SyntaqliteConflictAction onconf,
    SyntaqliteSortOrder sort_order,
    SyntaqliteBool is_autoincrement,
    SyntaqliteSourceSpan collation_name,
    SyntaqliteGeneratedColumnStorage generated_storage,
    uint32_t default_expr,
    uint32_t check_expr,
    uint32_t generated_expr,
    uint32_t fk_clause
) {
    return synq_ast_build(ctx,
        &(SyntaqliteColumnConstraint){
            .tag = SYNTAQLITE_NODE_COLUMN_CONSTRAINT,
            .kind = kind,
            .constraint_name = constraint_name,
            .onconf = onconf,
            .sort_order = sort_order,
            .is_autoincrement = is_autoincrement,
            .collation_name = collation_name,
            .generated_storage = generated_storage,
            .default_expr = default_expr,
            .check_expr = check_expr,
            .generated_expr = generated_expr,
            .fk_clause = fk_clause
        }, (uint32_t)sizeof(SyntaqliteColumnConstraint));
}

static inline uint32_t synq_ast_column_constraint_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_COLUMN_CONSTRAINT_LIST, list_id, child);
}

static inline uint32_t synq_ast_column_def(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan column_name,
    SyntaqliteSourceSpan type_name,
    uint32_t constraints
) {
    return synq_ast_build(ctx,
        &(SyntaqliteColumnDef){
            .tag = SYNTAQLITE_NODE_COLUMN_DEF,
            .column_name = column_name,
            .type_name = type_name,
            .constraints = constraints
        }, (uint32_t)sizeof(SyntaqliteColumnDef));
}

static inline uint32_t synq_ast_column_def_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_COLUMN_DEF_LIST, list_id, child);
}

static inline uint32_t synq_ast_table_constraint(
    SynqAstContext *ctx,
    SyntaqliteTableConstraintKind kind,
    SyntaqliteSourceSpan constraint_name,
    SyntaqliteConflictAction onconf,
    SyntaqliteBool is_autoincrement,
    uint32_t columns,
    uint32_t check_expr,
    uint32_t fk_clause
) {
    return synq_ast_build(ctx,
        &(SyntaqliteTableConstraint){
            .tag = SYNTAQLITE_NODE_TABLE_CONSTRAINT,
            .kind = kind,
            .constraint_name = constraint_name,
            .onconf = onconf,
            .is_autoincrement = is_autoincrement,
            .columns = columns,
            .check_expr = check_expr,
            .fk_clause = fk_clause
        }, (uint32_t)sizeof(SyntaqliteTableConstraint));
}

static inline uint32_t synq_ast_table_constraint_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_TABLE_CONSTRAINT_LIST, list_id, child);
}

static inline uint32_t synq_ast_create_table_stmt(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan table_name,
    SyntaqliteSourceSpan schema,
    SyntaqliteBool is_temp,
    SyntaqliteBool if_not_exists,
    SyntaqliteCreateTableStmtFlags flags,
    uint32_t columns,
    uint32_t table_constraints,
    uint32_t as_select
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCreateTableStmt){
            .tag = SYNTAQLITE_NODE_CREATE_TABLE_STMT,
            .table_name = table_name,
            .schema = schema,
            .is_temp = is_temp,
            .if_not_exists = if_not_exists,
            .flags = flags,
            .columns = columns,
            .table_constraints = table_constraints,
            .as_select = as_select
        }, (uint32_t)sizeof(SyntaqliteCreateTableStmt));
}

static inline uint32_t synq_ast_cte_definition(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan cte_name,
    SyntaqliteMaterialized materialized,
    uint32_t columns,
    uint32_t select
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCteDefinition){
            .tag = SYNTAQLITE_NODE_CTE_DEFINITION,
            .cte_name = cte_name,
            .materialized = materialized,
            .columns = columns,
            .select = select
        }, (uint32_t)sizeof(SyntaqliteCteDefinition));
}

static inline uint32_t synq_ast_cte_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_CTE_LIST, list_id, child);
}

static inline uint32_t synq_ast_with_clause(
    SynqAstContext *ctx,
    SyntaqliteBool recursive,
    uint32_t ctes,
    uint32_t select
) {
    return synq_ast_build(ctx,
        &(SyntaqliteWithClause){
            .tag = SYNTAQLITE_NODE_WITH_CLAUSE,
            .recursive = recursive,
            .ctes = ctes,
            .select = select
        }, (uint32_t)sizeof(SyntaqliteWithClause));
}

static inline uint32_t synq_ast_delete_stmt(
    SynqAstContext *ctx,
    uint32_t table,
    uint32_t where
) {
    return synq_ast_build(ctx,
        &(SyntaqliteDeleteStmt){
            .tag = SYNTAQLITE_NODE_DELETE_STMT,
            .table = table,
            .where = where
        }, (uint32_t)sizeof(SyntaqliteDeleteStmt));
}

static inline uint32_t synq_ast_set_clause(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan column,
    uint32_t columns,
    uint32_t value
) {
    return synq_ast_build(ctx,
        &(SyntaqliteSetClause){
            .tag = SYNTAQLITE_NODE_SET_CLAUSE,
            .column = column,
            .columns = columns,
            .value = value
        }, (uint32_t)sizeof(SyntaqliteSetClause));
}

static inline uint32_t synq_ast_set_clause_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_SET_CLAUSE_LIST, list_id, child);
}

static inline uint32_t synq_ast_update_stmt(
    SynqAstContext *ctx,
    SyntaqliteConflictAction conflict_action,
    uint32_t table,
    uint32_t setlist,
    uint32_t from_clause,
    uint32_t where
) {
    return synq_ast_build(ctx,
        &(SyntaqliteUpdateStmt){
            .tag = SYNTAQLITE_NODE_UPDATE_STMT,
            .conflict_action = conflict_action,
            .table = table,
            .setlist = setlist,
            .from_clause = from_clause,
            .where = where
        }, (uint32_t)sizeof(SyntaqliteUpdateStmt));
}

static inline uint32_t synq_ast_insert_stmt(
    SynqAstContext *ctx,
    SyntaqliteConflictAction conflict_action,
    uint32_t table,
    uint32_t columns,
    uint32_t source
) {
    return synq_ast_build(ctx,
        &(SyntaqliteInsertStmt){
            .tag = SYNTAQLITE_NODE_INSERT_STMT,
            .conflict_action = conflict_action,
            .table = table,
            .columns = columns,
            .source = source
        }, (uint32_t)sizeof(SyntaqliteInsertStmt));
}

static inline uint32_t synq_ast_binary_expr(
    SynqAstContext *ctx,
    SyntaqliteBinaryOp op,
    uint32_t left,
    uint32_t right
) {
    return synq_ast_build(ctx,
        &(SyntaqliteBinaryExpr){
            .tag = SYNTAQLITE_NODE_BINARY_EXPR,
            .op = op,
            .left = left,
            .right = right
        }, (uint32_t)sizeof(SyntaqliteBinaryExpr));
}

static inline uint32_t synq_ast_unary_expr(
    SynqAstContext *ctx,
    SyntaqliteUnaryOp op,
    uint32_t operand
) {
    return synq_ast_build(ctx,
        &(SyntaqliteUnaryExpr){
            .tag = SYNTAQLITE_NODE_UNARY_EXPR,
            .op = op,
            .operand = operand
        }, (uint32_t)sizeof(SyntaqliteUnaryExpr));
}

static inline uint32_t synq_ast_literal(
    SynqAstContext *ctx,
    SyntaqliteLiteralType literal_type,
    SyntaqliteSourceSpan source
) {
    return synq_ast_build(ctx,
        &(SyntaqliteLiteral){
            .tag = SYNTAQLITE_NODE_LITERAL,
            .literal_type = literal_type,
            .source = source
        }, (uint32_t)sizeof(SyntaqliteLiteral));
}

static inline uint32_t synq_ast_expr_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_EXPR_LIST, list_id, child);
}

static inline uint32_t synq_ast_function_call(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan func_name,
    SyntaqliteFunctionCallFlags flags,
    uint32_t args,
    uint32_t filter_clause,
    uint32_t over_clause
) {
    return synq_ast_build(ctx,
        &(SyntaqliteFunctionCall){
            .tag = SYNTAQLITE_NODE_FUNCTION_CALL,
            .func_name = func_name,
            .flags = flags,
            .args = args,
            .filter_clause = filter_clause,
            .over_clause = over_clause
        }, (uint32_t)sizeof(SyntaqliteFunctionCall));
}

static inline uint32_t synq_ast_variable(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan source
) {
    return synq_ast_build(ctx,
        &(SyntaqliteVariable){
            .tag = SYNTAQLITE_NODE_VARIABLE,
            .source = source
        }, (uint32_t)sizeof(SyntaqliteVariable));
}

static inline uint32_t synq_ast_collate_expr(
    SynqAstContext *ctx,
    uint32_t expr,
    SyntaqliteSourceSpan collation
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCollateExpr){
            .tag = SYNTAQLITE_NODE_COLLATE_EXPR,
            .expr = expr,
            .collation = collation
        }, (uint32_t)sizeof(SyntaqliteCollateExpr));
}

static inline uint32_t synq_ast_raise_expr(
    SynqAstContext *ctx,
    SyntaqliteRaiseType raise_type,
    uint32_t error_message
) {
    return synq_ast_build(ctx,
        &(SyntaqliteRaiseExpr){
            .tag = SYNTAQLITE_NODE_RAISE_EXPR,
            .raise_type = raise_type,
            .error_message = error_message
        }, (uint32_t)sizeof(SyntaqliteRaiseExpr));
}

static inline uint32_t synq_ast_qualified_name(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan object_name,
    SyntaqliteSourceSpan schema
) {
    return synq_ast_build(ctx,
        &(SyntaqliteQualifiedName){
            .tag = SYNTAQLITE_NODE_QUALIFIED_NAME,
            .object_name = object_name,
            .schema = schema
        }, (uint32_t)sizeof(SyntaqliteQualifiedName));
}

static inline uint32_t synq_ast_drop_stmt(
    SynqAstContext *ctx,
    SyntaqliteDropObjectType object_type,
    SyntaqliteBool if_exists,
    uint32_t target
) {
    return synq_ast_build(ctx,
        &(SyntaqliteDropStmt){
            .tag = SYNTAQLITE_NODE_DROP_STMT,
            .object_type = object_type,
            .if_exists = if_exists,
            .target = target
        }, (uint32_t)sizeof(SyntaqliteDropStmt));
}

static inline uint32_t synq_ast_alter_table_stmt(
    SynqAstContext *ctx,
    SyntaqliteAlterOp op,
    uint32_t target,
    SyntaqliteSourceSpan new_name,
    SyntaqliteSourceSpan old_name
) {
    return synq_ast_build(ctx,
        &(SyntaqliteAlterTableStmt){
            .tag = SYNTAQLITE_NODE_ALTER_TABLE_STMT,
            .op = op,
            .target = target,
            .new_name = new_name,
            .old_name = old_name
        }, (uint32_t)sizeof(SyntaqliteAlterTableStmt));
}

static inline uint32_t synq_ast_transaction_stmt(
    SynqAstContext *ctx,
    SyntaqliteTransactionOp op,
    SyntaqliteTransactionType trans_type
) {
    return synq_ast_build(ctx,
        &(SyntaqliteTransactionStmt){
            .tag = SYNTAQLITE_NODE_TRANSACTION_STMT,
            .op = op,
            .trans_type = trans_type
        }, (uint32_t)sizeof(SyntaqliteTransactionStmt));
}

static inline uint32_t synq_ast_savepoint_stmt(
    SynqAstContext *ctx,
    SyntaqliteSavepointOp op,
    SyntaqliteSourceSpan savepoint_name
) {
    return synq_ast_build(ctx,
        &(SyntaqliteSavepointStmt){
            .tag = SYNTAQLITE_NODE_SAVEPOINT_STMT,
            .op = op,
            .savepoint_name = savepoint_name
        }, (uint32_t)sizeof(SyntaqliteSavepointStmt));
}

static inline uint32_t synq_ast_result_column(
    SynqAstContext *ctx,
    SyntaqliteResultColumnFlags flags,
    SyntaqliteSourceSpan alias,
    uint32_t expr
) {
    return synq_ast_build(ctx,
        &(SyntaqliteResultColumn){
            .tag = SYNTAQLITE_NODE_RESULT_COLUMN,
            .flags = flags,
            .alias = alias,
            .expr = expr
        }, (uint32_t)sizeof(SyntaqliteResultColumn));
}

static inline uint32_t synq_ast_result_column_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_RESULT_COLUMN_LIST, list_id, child);
}

static inline uint32_t synq_ast_select_stmt(
    SynqAstContext *ctx,
    SyntaqliteSelectStmtFlags flags,
    uint32_t columns,
    uint32_t from_clause,
    uint32_t where,
    uint32_t groupby,
    uint32_t having,
    uint32_t orderby,
    uint32_t limit_clause,
    uint32_t window_clause
) {
    return synq_ast_build(ctx,
        &(SyntaqliteSelectStmt){
            .tag = SYNTAQLITE_NODE_SELECT_STMT,
            .flags = flags,
            .columns = columns,
            .from_clause = from_clause,
            .where = where,
            .groupby = groupby,
            .having = having,
            .orderby = orderby,
            .limit_clause = limit_clause,
            .window_clause = window_clause
        }, (uint32_t)sizeof(SyntaqliteSelectStmt));
}

static inline uint32_t synq_ast_ordering_term(
    SynqAstContext *ctx,
    uint32_t expr,
    SyntaqliteSortOrder sort_order,
    SyntaqliteNullsOrder nulls_order
) {
    return synq_ast_build(ctx,
        &(SyntaqliteOrderingTerm){
            .tag = SYNTAQLITE_NODE_ORDERING_TERM,
            .expr = expr,
            .sort_order = sort_order,
            .nulls_order = nulls_order
        }, (uint32_t)sizeof(SyntaqliteOrderingTerm));
}

static inline uint32_t synq_ast_order_by_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_ORDER_BY_LIST, list_id, child);
}

static inline uint32_t synq_ast_limit_clause(
    SynqAstContext *ctx,
    uint32_t limit,
    uint32_t offset
) {
    return synq_ast_build(ctx,
        &(SyntaqliteLimitClause){
            .tag = SYNTAQLITE_NODE_LIMIT_CLAUSE,
            .limit = limit,
            .offset = offset
        }, (uint32_t)sizeof(SyntaqliteLimitClause));
}

static inline uint32_t synq_ast_table_ref(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan table_name,
    SyntaqliteSourceSpan schema,
    SyntaqliteSourceSpan alias
) {
    return synq_ast_build(ctx,
        &(SyntaqliteTableRef){
            .tag = SYNTAQLITE_NODE_TABLE_REF,
            .table_name = table_name,
            .schema = schema,
            .alias = alias
        }, (uint32_t)sizeof(SyntaqliteTableRef));
}

static inline uint32_t synq_ast_subquery_table_source(
    SynqAstContext *ctx,
    uint32_t select,
    SyntaqliteSourceSpan alias
) {
    return synq_ast_build(ctx,
        &(SyntaqliteSubqueryTableSource){
            .tag = SYNTAQLITE_NODE_SUBQUERY_TABLE_SOURCE,
            .select = select,
            .alias = alias
        }, (uint32_t)sizeof(SyntaqliteSubqueryTableSource));
}

static inline uint32_t synq_ast_join_clause(
    SynqAstContext *ctx,
    SyntaqliteJoinType join_type,
    uint32_t left,
    uint32_t right,
    uint32_t on_expr,
    uint32_t using_columns
) {
    return synq_ast_build(ctx,
        &(SyntaqliteJoinClause){
            .tag = SYNTAQLITE_NODE_JOIN_CLAUSE,
            .join_type = join_type,
            .left = left,
            .right = right,
            .on_expr = on_expr,
            .using_columns = using_columns
        }, (uint32_t)sizeof(SyntaqliteJoinClause));
}

static inline uint32_t synq_ast_join_prefix(
    SynqAstContext *ctx,
    uint32_t source,
    SyntaqliteJoinType join_type
) {
    return synq_ast_build(ctx,
        &(SyntaqliteJoinPrefix){
            .tag = SYNTAQLITE_NODE_JOIN_PREFIX,
            .source = source,
            .join_type = join_type
        }, (uint32_t)sizeof(SyntaqliteJoinPrefix));
}

static inline uint32_t synq_ast_trigger_event(
    SynqAstContext *ctx,
    SyntaqliteTriggerEventType event_type,
    uint32_t columns
) {
    return synq_ast_build(ctx,
        &(SyntaqliteTriggerEvent){
            .tag = SYNTAQLITE_NODE_TRIGGER_EVENT,
            .event_type = event_type,
            .columns = columns
        }, (uint32_t)sizeof(SyntaqliteTriggerEvent));
}

static inline uint32_t synq_ast_trigger_cmd_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_TRIGGER_CMD_LIST, list_id, child);
}

static inline uint32_t synq_ast_create_trigger_stmt(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan trigger_name,
    SyntaqliteSourceSpan schema,
    SyntaqliteBool is_temp,
    SyntaqliteBool if_not_exists,
    SyntaqliteTriggerTiming timing,
    uint32_t event,
    uint32_t table,
    uint32_t when_expr,
    uint32_t body
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCreateTriggerStmt){
            .tag = SYNTAQLITE_NODE_CREATE_TRIGGER_STMT,
            .trigger_name = trigger_name,
            .schema = schema,
            .is_temp = is_temp,
            .if_not_exists = if_not_exists,
            .timing = timing,
            .event = event,
            .table = table,
            .when_expr = when_expr,
            .body = body
        }, (uint32_t)sizeof(SyntaqliteCreateTriggerStmt));
}

static inline uint32_t synq_ast_create_virtual_table_stmt(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan table_name,
    SyntaqliteSourceSpan schema,
    SyntaqliteSourceSpan module_name,
    SyntaqliteBool if_not_exists,
    SyntaqliteSourceSpan module_args
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCreateVirtualTableStmt){
            .tag = SYNTAQLITE_NODE_CREATE_VIRTUAL_TABLE_STMT,
            .table_name = table_name,
            .schema = schema,
            .module_name = module_name,
            .if_not_exists = if_not_exists,
            .module_args = module_args
        }, (uint32_t)sizeof(SyntaqliteCreateVirtualTableStmt));
}

static inline uint32_t synq_ast_pragma_stmt(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan pragma_name,
    SyntaqliteSourceSpan schema,
    SyntaqliteSourceSpan value,
    SyntaqlitePragmaForm pragma_form
) {
    return synq_ast_build(ctx,
        &(SyntaqlitePragmaStmt){
            .tag = SYNTAQLITE_NODE_PRAGMA_STMT,
            .pragma_name = pragma_name,
            .schema = schema,
            .value = value,
            .pragma_form = pragma_form
        }, (uint32_t)sizeof(SyntaqlitePragmaStmt));
}

static inline uint32_t synq_ast_analyze_stmt(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan target_name,
    SyntaqliteSourceSpan schema,
    SyntaqliteAnalyzeKind kind
) {
    return synq_ast_build(ctx,
        &(SyntaqliteAnalyzeStmt){
            .tag = SYNTAQLITE_NODE_ANALYZE_STMT,
            .target_name = target_name,
            .schema = schema,
            .kind = kind
        }, (uint32_t)sizeof(SyntaqliteAnalyzeStmt));
}

static inline uint32_t synq_ast_attach_stmt(
    SynqAstContext *ctx,
    uint32_t filename,
    uint32_t db_name,
    uint32_t key
) {
    return synq_ast_build(ctx,
        &(SyntaqliteAttachStmt){
            .tag = SYNTAQLITE_NODE_ATTACH_STMT,
            .filename = filename,
            .db_name = db_name,
            .key = key
        }, (uint32_t)sizeof(SyntaqliteAttachStmt));
}

static inline uint32_t synq_ast_detach_stmt(
    SynqAstContext *ctx,
    uint32_t db_name
) {
    return synq_ast_build(ctx,
        &(SyntaqliteDetachStmt){
            .tag = SYNTAQLITE_NODE_DETACH_STMT,
            .db_name = db_name
        }, (uint32_t)sizeof(SyntaqliteDetachStmt));
}

static inline uint32_t synq_ast_vacuum_stmt(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan schema,
    uint32_t into_expr
) {
    return synq_ast_build(ctx,
        &(SyntaqliteVacuumStmt){
            .tag = SYNTAQLITE_NODE_VACUUM_STMT,
            .schema = schema,
            .into_expr = into_expr
        }, (uint32_t)sizeof(SyntaqliteVacuumStmt));
}

static inline uint32_t synq_ast_explain_stmt(
    SynqAstContext *ctx,
    SyntaqliteExplainMode explain_mode,
    uint32_t stmt
) {
    return synq_ast_build(ctx,
        &(SyntaqliteExplainStmt){
            .tag = SYNTAQLITE_NODE_EXPLAIN_STMT,
            .explain_mode = explain_mode,
            .stmt = stmt
        }, (uint32_t)sizeof(SyntaqliteExplainStmt));
}

static inline uint32_t synq_ast_create_index_stmt(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan index_name,
    SyntaqliteSourceSpan schema,
    SyntaqliteSourceSpan table_name,
    SyntaqliteBool is_unique,
    SyntaqliteBool if_not_exists,
    uint32_t columns,
    uint32_t where
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCreateIndexStmt){
            .tag = SYNTAQLITE_NODE_CREATE_INDEX_STMT,
            .index_name = index_name,
            .schema = schema,
            .table_name = table_name,
            .is_unique = is_unique,
            .if_not_exists = if_not_exists,
            .columns = columns,
            .where = where
        }, (uint32_t)sizeof(SyntaqliteCreateIndexStmt));
}

static inline uint32_t synq_ast_create_view_stmt(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan view_name,
    SyntaqliteSourceSpan schema,
    SyntaqliteBool is_temp,
    SyntaqliteBool if_not_exists,
    uint32_t column_names,
    uint32_t select
) {
    return synq_ast_build(ctx,
        &(SyntaqliteCreateViewStmt){
            .tag = SYNTAQLITE_NODE_CREATE_VIEW_STMT,
            .view_name = view_name,
            .schema = schema,
            .is_temp = is_temp,
            .if_not_exists = if_not_exists,
            .column_names = column_names,
            .select = select
        }, (uint32_t)sizeof(SyntaqliteCreateViewStmt));
}

static inline uint32_t synq_ast_values_row_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_VALUES_ROW_LIST, list_id, child);
}

static inline uint32_t synq_ast_values_clause(
    SynqAstContext *ctx,
    uint32_t rows
) {
    return synq_ast_build(ctx,
        &(SyntaqliteValuesClause){
            .tag = SYNTAQLITE_NODE_VALUES_CLAUSE,
            .rows = rows
        }, (uint32_t)sizeof(SyntaqliteValuesClause));
}

static inline uint32_t synq_ast_frame_bound(
    SynqAstContext *ctx,
    SyntaqliteFrameBoundType bound_type,
    uint32_t expr
) {
    return synq_ast_build(ctx,
        &(SyntaqliteFrameBound){
            .tag = SYNTAQLITE_NODE_FRAME_BOUND,
            .bound_type = bound_type,
            .expr = expr
        }, (uint32_t)sizeof(SyntaqliteFrameBound));
}

static inline uint32_t synq_ast_frame_spec(
    SynqAstContext *ctx,
    SyntaqliteFrameType frame_type,
    SyntaqliteFrameExclude exclude,
    uint32_t start_bound,
    uint32_t end_bound
) {
    return synq_ast_build(ctx,
        &(SyntaqliteFrameSpec){
            .tag = SYNTAQLITE_NODE_FRAME_SPEC,
            .frame_type = frame_type,
            .exclude = exclude,
            .start_bound = start_bound,
            .end_bound = end_bound
        }, (uint32_t)sizeof(SyntaqliteFrameSpec));
}

static inline uint32_t synq_ast_window_def(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan base_window_name,
    uint32_t partition_by,
    uint32_t orderby,
    uint32_t frame
) {
    return synq_ast_build(ctx,
        &(SyntaqliteWindowDef){
            .tag = SYNTAQLITE_NODE_WINDOW_DEF,
            .base_window_name = base_window_name,
            .partition_by = partition_by,
            .orderby = orderby,
            .frame = frame
        }, (uint32_t)sizeof(SyntaqliteWindowDef));
}

static inline uint32_t synq_ast_window_def_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_WINDOW_DEF_LIST, list_id, child);
}

static inline uint32_t synq_ast_named_window_def(
    SynqAstContext *ctx,
    SyntaqliteSourceSpan window_name,
    uint32_t window_def
) {
    return synq_ast_build(ctx,
        &(SyntaqliteNamedWindowDef){
            .tag = SYNTAQLITE_NODE_NAMED_WINDOW_DEF,
            .window_name = window_name,
            .window_def = window_def
        }, (uint32_t)sizeof(SyntaqliteNamedWindowDef));
}

static inline uint32_t synq_ast_named_window_def_list(
    SynqAstContext *ctx,
    uint32_t list_id,
    uint32_t child
) {
    return synq_ast_list_append(ctx, SYNTAQLITE_NODE_NAMED_WINDOW_DEF_LIST, list_id, child);
}

static inline uint32_t synq_ast_filter_over(
    SynqAstContext *ctx,
    uint32_t filter_expr,
    uint32_t over_def,
    SyntaqliteSourceSpan over_name
) {
    return synq_ast_build(ctx,
        &(SyntaqliteFilterOver){
            .tag = SYNTAQLITE_NODE_FILTER_OVER,
            .filter_expr = filter_expr,
            .over_def = over_def,
            .over_name = over_name
        }, (uint32_t)sizeof(SyntaqliteFilterOver));
}

// ============ Range Field Metadata ============

typedef struct { uint16_t offset; uint8_t kind; } SynqFieldRangeMeta;

static const SynqFieldRangeMeta range_meta_aggregate_function_call[] = {
    {offsetof(SyntaqliteAggregateFunctionCall, func_name), 1},
    {offsetof(SyntaqliteAggregateFunctionCall, args), 0},
    {offsetof(SyntaqliteAggregateFunctionCall, orderby), 0},
    {offsetof(SyntaqliteAggregateFunctionCall, filter_clause), 0},
    {offsetof(SyntaqliteAggregateFunctionCall, over_clause), 0},
};

static const SynqFieldRangeMeta range_meta_cast_expr[] = {
    {offsetof(SyntaqliteCastExpr, expr), 0},
    {offsetof(SyntaqliteCastExpr, type_name), 1},
};

static const SynqFieldRangeMeta range_meta_column_ref[] = {
    {offsetof(SyntaqliteColumnRef, column), 1},
    {offsetof(SyntaqliteColumnRef, table), 1},
    {offsetof(SyntaqliteColumnRef, schema), 1},
};

static const SynqFieldRangeMeta range_meta_compound_select[] = {
    {offsetof(SyntaqliteCompoundSelect, left), 0},
    {offsetof(SyntaqliteCompoundSelect, right), 0},
};

static const SynqFieldRangeMeta range_meta_subquery_expr[] = {
    {offsetof(SyntaqliteSubqueryExpr, select), 0},
};

static const SynqFieldRangeMeta range_meta_exists_expr[] = {
    {offsetof(SyntaqliteExistsExpr, select), 0},
};

static const SynqFieldRangeMeta range_meta_in_expr[] = {
    {offsetof(SyntaqliteInExpr, operand), 0},
    {offsetof(SyntaqliteInExpr, source), 0},
};

static const SynqFieldRangeMeta range_meta_is_expr[] = {
    {offsetof(SyntaqliteIsExpr, left), 0},
    {offsetof(SyntaqliteIsExpr, right), 0},
};

static const SynqFieldRangeMeta range_meta_between_expr[] = {
    {offsetof(SyntaqliteBetweenExpr, operand), 0},
    {offsetof(SyntaqliteBetweenExpr, low), 0},
    {offsetof(SyntaqliteBetweenExpr, high), 0},
};

static const SynqFieldRangeMeta range_meta_like_expr[] = {
    {offsetof(SyntaqliteLikeExpr, operand), 0},
    {offsetof(SyntaqliteLikeExpr, pattern), 0},
    {offsetof(SyntaqliteLikeExpr, escape), 0},
};

static const SynqFieldRangeMeta range_meta_case_expr[] = {
    {offsetof(SyntaqliteCaseExpr, operand), 0},
    {offsetof(SyntaqliteCaseExpr, else_expr), 0},
    {offsetof(SyntaqliteCaseExpr, whens), 0},
};

static const SynqFieldRangeMeta range_meta_case_when[] = {
    {offsetof(SyntaqliteCaseWhen, when_expr), 0},
    {offsetof(SyntaqliteCaseWhen, then_expr), 0},
};

static const SynqFieldRangeMeta range_meta_foreign_key_clause[] = {
    {offsetof(SyntaqliteForeignKeyClause, ref_table), 1},
    {offsetof(SyntaqliteForeignKeyClause, ref_columns), 0},
};

static const SynqFieldRangeMeta range_meta_column_constraint[] = {
    {offsetof(SyntaqliteColumnConstraint, constraint_name), 1},
    {offsetof(SyntaqliteColumnConstraint, collation_name), 1},
    {offsetof(SyntaqliteColumnConstraint, default_expr), 0},
    {offsetof(SyntaqliteColumnConstraint, check_expr), 0},
    {offsetof(SyntaqliteColumnConstraint, generated_expr), 0},
    {offsetof(SyntaqliteColumnConstraint, fk_clause), 0},
};

static const SynqFieldRangeMeta range_meta_column_def[] = {
    {offsetof(SyntaqliteColumnDef, column_name), 1},
    {offsetof(SyntaqliteColumnDef, type_name), 1},
    {offsetof(SyntaqliteColumnDef, constraints), 0},
};

static const SynqFieldRangeMeta range_meta_table_constraint[] = {
    {offsetof(SyntaqliteTableConstraint, constraint_name), 1},
    {offsetof(SyntaqliteTableConstraint, columns), 0},
    {offsetof(SyntaqliteTableConstraint, check_expr), 0},
    {offsetof(SyntaqliteTableConstraint, fk_clause), 0},
};

static const SynqFieldRangeMeta range_meta_create_table_stmt[] = {
    {offsetof(SyntaqliteCreateTableStmt, table_name), 1},
    {offsetof(SyntaqliteCreateTableStmt, schema), 1},
    {offsetof(SyntaqliteCreateTableStmt, columns), 0},
    {offsetof(SyntaqliteCreateTableStmt, table_constraints), 0},
    {offsetof(SyntaqliteCreateTableStmt, as_select), 0},
};

static const SynqFieldRangeMeta range_meta_cte_definition[] = {
    {offsetof(SyntaqliteCteDefinition, cte_name), 1},
    {offsetof(SyntaqliteCteDefinition, columns), 0},
    {offsetof(SyntaqliteCteDefinition, select), 0},
};

static const SynqFieldRangeMeta range_meta_with_clause[] = {
    {offsetof(SyntaqliteWithClause, ctes), 0},
    {offsetof(SyntaqliteWithClause, select), 0},
};

static const SynqFieldRangeMeta range_meta_delete_stmt[] = {
    {offsetof(SyntaqliteDeleteStmt, table), 0},
    {offsetof(SyntaqliteDeleteStmt, where), 0},
};

static const SynqFieldRangeMeta range_meta_set_clause[] = {
    {offsetof(SyntaqliteSetClause, column), 1},
    {offsetof(SyntaqliteSetClause, columns), 0},
    {offsetof(SyntaqliteSetClause, value), 0},
};

static const SynqFieldRangeMeta range_meta_update_stmt[] = {
    {offsetof(SyntaqliteUpdateStmt, table), 0},
    {offsetof(SyntaqliteUpdateStmt, setlist), 0},
    {offsetof(SyntaqliteUpdateStmt, from_clause), 0},
    {offsetof(SyntaqliteUpdateStmt, where), 0},
};

static const SynqFieldRangeMeta range_meta_insert_stmt[] = {
    {offsetof(SyntaqliteInsertStmt, table), 0},
    {offsetof(SyntaqliteInsertStmt, columns), 0},
    {offsetof(SyntaqliteInsertStmt, source), 0},
};

static const SynqFieldRangeMeta range_meta_binary_expr[] = {
    {offsetof(SyntaqliteBinaryExpr, left), 0},
    {offsetof(SyntaqliteBinaryExpr, right), 0},
};

static const SynqFieldRangeMeta range_meta_unary_expr[] = {
    {offsetof(SyntaqliteUnaryExpr, operand), 0},
};

static const SynqFieldRangeMeta range_meta_literal[] = {
    {offsetof(SyntaqliteLiteral, source), 1},
};

static const SynqFieldRangeMeta range_meta_function_call[] = {
    {offsetof(SyntaqliteFunctionCall, func_name), 1},
    {offsetof(SyntaqliteFunctionCall, args), 0},
    {offsetof(SyntaqliteFunctionCall, filter_clause), 0},
    {offsetof(SyntaqliteFunctionCall, over_clause), 0},
};

static const SynqFieldRangeMeta range_meta_variable[] = {
    {offsetof(SyntaqliteVariable, source), 1},
};

static const SynqFieldRangeMeta range_meta_collate_expr[] = {
    {offsetof(SyntaqliteCollateExpr, expr), 0},
    {offsetof(SyntaqliteCollateExpr, collation), 1},
};

static const SynqFieldRangeMeta range_meta_raise_expr[] = {
    {offsetof(SyntaqliteRaiseExpr, error_message), 0},
};

static const SynqFieldRangeMeta range_meta_qualified_name[] = {
    {offsetof(SyntaqliteQualifiedName, object_name), 1},
    {offsetof(SyntaqliteQualifiedName, schema), 1},
};

static const SynqFieldRangeMeta range_meta_drop_stmt[] = {
    {offsetof(SyntaqliteDropStmt, target), 0},
};

static const SynqFieldRangeMeta range_meta_alter_table_stmt[] = {
    {offsetof(SyntaqliteAlterTableStmt, target), 0},
    {offsetof(SyntaqliteAlterTableStmt, new_name), 1},
    {offsetof(SyntaqliteAlterTableStmt, old_name), 1},
};

static const SynqFieldRangeMeta range_meta_savepoint_stmt[] = {
    {offsetof(SyntaqliteSavepointStmt, savepoint_name), 1},
};

static const SynqFieldRangeMeta range_meta_result_column[] = {
    {offsetof(SyntaqliteResultColumn, alias), 1},
    {offsetof(SyntaqliteResultColumn, expr), 0},
};

static const SynqFieldRangeMeta range_meta_select_stmt[] = {
    {offsetof(SyntaqliteSelectStmt, columns), 0},
    {offsetof(SyntaqliteSelectStmt, from_clause), 0},
    {offsetof(SyntaqliteSelectStmt, where), 0},
    {offsetof(SyntaqliteSelectStmt, groupby), 0},
    {offsetof(SyntaqliteSelectStmt, having), 0},
    {offsetof(SyntaqliteSelectStmt, orderby), 0},
    {offsetof(SyntaqliteSelectStmt, limit_clause), 0},
    {offsetof(SyntaqliteSelectStmt, window_clause), 0},
};

static const SynqFieldRangeMeta range_meta_ordering_term[] = {
    {offsetof(SyntaqliteOrderingTerm, expr), 0},
};

static const SynqFieldRangeMeta range_meta_limit_clause[] = {
    {offsetof(SyntaqliteLimitClause, limit), 0},
    {offsetof(SyntaqliteLimitClause, offset), 0},
};

static const SynqFieldRangeMeta range_meta_table_ref[] = {
    {offsetof(SyntaqliteTableRef, table_name), 1},
    {offsetof(SyntaqliteTableRef, schema), 1},
    {offsetof(SyntaqliteTableRef, alias), 1},
};

static const SynqFieldRangeMeta range_meta_subquery_table_source[] = {
    {offsetof(SyntaqliteSubqueryTableSource, select), 0},
    {offsetof(SyntaqliteSubqueryTableSource, alias), 1},
};

static const SynqFieldRangeMeta range_meta_join_clause[] = {
    {offsetof(SyntaqliteJoinClause, left), 0},
    {offsetof(SyntaqliteJoinClause, right), 0},
    {offsetof(SyntaqliteJoinClause, on_expr), 0},
    {offsetof(SyntaqliteJoinClause, using_columns), 0},
};

static const SynqFieldRangeMeta range_meta_join_prefix[] = {
    {offsetof(SyntaqliteJoinPrefix, source), 0},
};

static const SynqFieldRangeMeta range_meta_trigger_event[] = {
    {offsetof(SyntaqliteTriggerEvent, columns), 0},
};

static const SynqFieldRangeMeta range_meta_create_trigger_stmt[] = {
    {offsetof(SyntaqliteCreateTriggerStmt, trigger_name), 1},
    {offsetof(SyntaqliteCreateTriggerStmt, schema), 1},
    {offsetof(SyntaqliteCreateTriggerStmt, event), 0},
    {offsetof(SyntaqliteCreateTriggerStmt, table), 0},
    {offsetof(SyntaqliteCreateTriggerStmt, when_expr), 0},
    {offsetof(SyntaqliteCreateTriggerStmt, body), 0},
};

static const SynqFieldRangeMeta range_meta_create_virtual_table_stmt[] = {
    {offsetof(SyntaqliteCreateVirtualTableStmt, table_name), 1},
    {offsetof(SyntaqliteCreateVirtualTableStmt, schema), 1},
    {offsetof(SyntaqliteCreateVirtualTableStmt, module_name), 1},
    {offsetof(SyntaqliteCreateVirtualTableStmt, module_args), 1},
};

static const SynqFieldRangeMeta range_meta_pragma_stmt[] = {
    {offsetof(SyntaqlitePragmaStmt, pragma_name), 1},
    {offsetof(SyntaqlitePragmaStmt, schema), 1},
    {offsetof(SyntaqlitePragmaStmt, value), 1},
};

static const SynqFieldRangeMeta range_meta_analyze_stmt[] = {
    {offsetof(SyntaqliteAnalyzeStmt, target_name), 1},
    {offsetof(SyntaqliteAnalyzeStmt, schema), 1},
};

static const SynqFieldRangeMeta range_meta_attach_stmt[] = {
    {offsetof(SyntaqliteAttachStmt, filename), 0},
    {offsetof(SyntaqliteAttachStmt, db_name), 0},
    {offsetof(SyntaqliteAttachStmt, key), 0},
};

static const SynqFieldRangeMeta range_meta_detach_stmt[] = {
    {offsetof(SyntaqliteDetachStmt, db_name), 0},
};

static const SynqFieldRangeMeta range_meta_vacuum_stmt[] = {
    {offsetof(SyntaqliteVacuumStmt, schema), 1},
    {offsetof(SyntaqliteVacuumStmt, into_expr), 0},
};

static const SynqFieldRangeMeta range_meta_explain_stmt[] = {
    {offsetof(SyntaqliteExplainStmt, stmt), 0},
};

static const SynqFieldRangeMeta range_meta_create_index_stmt[] = {
    {offsetof(SyntaqliteCreateIndexStmt, index_name), 1},
    {offsetof(SyntaqliteCreateIndexStmt, schema), 1},
    {offsetof(SyntaqliteCreateIndexStmt, table_name), 1},
    {offsetof(SyntaqliteCreateIndexStmt, columns), 0},
    {offsetof(SyntaqliteCreateIndexStmt, where), 0},
};

static const SynqFieldRangeMeta range_meta_create_view_stmt[] = {
    {offsetof(SyntaqliteCreateViewStmt, view_name), 1},
    {offsetof(SyntaqliteCreateViewStmt, schema), 1},
    {offsetof(SyntaqliteCreateViewStmt, column_names), 0},
    {offsetof(SyntaqliteCreateViewStmt, select), 0},
};

static const SynqFieldRangeMeta range_meta_values_clause[] = {
    {offsetof(SyntaqliteValuesClause, rows), 0},
};

static const SynqFieldRangeMeta range_meta_frame_bound[] = {
    {offsetof(SyntaqliteFrameBound, expr), 0},
};

static const SynqFieldRangeMeta range_meta_frame_spec[] = {
    {offsetof(SyntaqliteFrameSpec, start_bound), 0},
    {offsetof(SyntaqliteFrameSpec, end_bound), 0},
};

static const SynqFieldRangeMeta range_meta_window_def[] = {
    {offsetof(SyntaqliteWindowDef, base_window_name), 1},
    {offsetof(SyntaqliteWindowDef, partition_by), 0},
    {offsetof(SyntaqliteWindowDef, orderby), 0},
    {offsetof(SyntaqliteWindowDef, frame), 0},
};

static const SynqFieldRangeMeta range_meta_named_window_def[] = {
    {offsetof(SyntaqliteNamedWindowDef, window_name), 1},
    {offsetof(SyntaqliteNamedWindowDef, window_def), 0},
};

static const SynqFieldRangeMeta range_meta_filter_over[] = {
    {offsetof(SyntaqliteFilterOver, filter_expr), 0},
    {offsetof(SyntaqliteFilterOver, over_def), 0},
    {offsetof(SyntaqliteFilterOver, over_name), 1},
};

static const struct { const SynqFieldRangeMeta *fields; uint8_t count; } range_meta_table[] = {
    [SYNTAQLITE_NODE_NULL] = {NULL, 0},
    [SYNTAQLITE_NODE_AGGREGATE_FUNCTION_CALL] = {range_meta_aggregate_function_call, 5},
    [SYNTAQLITE_NODE_CAST_EXPR] = {range_meta_cast_expr, 2},
    [SYNTAQLITE_NODE_COLUMN_REF] = {range_meta_column_ref, 3},
    [SYNTAQLITE_NODE_COMPOUND_SELECT] = {range_meta_compound_select, 2},
    [SYNTAQLITE_NODE_SUBQUERY_EXPR] = {range_meta_subquery_expr, 1},
    [SYNTAQLITE_NODE_EXISTS_EXPR] = {range_meta_exists_expr, 1},
    [SYNTAQLITE_NODE_IN_EXPR] = {range_meta_in_expr, 2},
    [SYNTAQLITE_NODE_IS_EXPR] = {range_meta_is_expr, 2},
    [SYNTAQLITE_NODE_BETWEEN_EXPR] = {range_meta_between_expr, 3},
    [SYNTAQLITE_NODE_LIKE_EXPR] = {range_meta_like_expr, 3},
    [SYNTAQLITE_NODE_CASE_EXPR] = {range_meta_case_expr, 3},
    [SYNTAQLITE_NODE_CASE_WHEN] = {range_meta_case_when, 2},
    [SYNTAQLITE_NODE_CASE_WHEN_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_FOREIGN_KEY_CLAUSE] = {range_meta_foreign_key_clause, 2},
    [SYNTAQLITE_NODE_COLUMN_CONSTRAINT] = {range_meta_column_constraint, 6},
    [SYNTAQLITE_NODE_COLUMN_CONSTRAINT_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_COLUMN_DEF] = {range_meta_column_def, 3},
    [SYNTAQLITE_NODE_COLUMN_DEF_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_TABLE_CONSTRAINT] = {range_meta_table_constraint, 4},
    [SYNTAQLITE_NODE_TABLE_CONSTRAINT_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_CREATE_TABLE_STMT] = {range_meta_create_table_stmt, 5},
    [SYNTAQLITE_NODE_CTE_DEFINITION] = {range_meta_cte_definition, 3},
    [SYNTAQLITE_NODE_CTE_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_WITH_CLAUSE] = {range_meta_with_clause, 2},
    [SYNTAQLITE_NODE_DELETE_STMT] = {range_meta_delete_stmt, 2},
    [SYNTAQLITE_NODE_SET_CLAUSE] = {range_meta_set_clause, 3},
    [SYNTAQLITE_NODE_SET_CLAUSE_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_UPDATE_STMT] = {range_meta_update_stmt, 4},
    [SYNTAQLITE_NODE_INSERT_STMT] = {range_meta_insert_stmt, 3},
    [SYNTAQLITE_NODE_BINARY_EXPR] = {range_meta_binary_expr, 2},
    [SYNTAQLITE_NODE_UNARY_EXPR] = {range_meta_unary_expr, 1},
    [SYNTAQLITE_NODE_LITERAL] = {range_meta_literal, 1},
    [SYNTAQLITE_NODE_EXPR_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_FUNCTION_CALL] = {range_meta_function_call, 4},
    [SYNTAQLITE_NODE_VARIABLE] = {range_meta_variable, 1},
    [SYNTAQLITE_NODE_COLLATE_EXPR] = {range_meta_collate_expr, 2},
    [SYNTAQLITE_NODE_RAISE_EXPR] = {range_meta_raise_expr, 1},
    [SYNTAQLITE_NODE_QUALIFIED_NAME] = {range_meta_qualified_name, 2},
    [SYNTAQLITE_NODE_DROP_STMT] = {range_meta_drop_stmt, 1},
    [SYNTAQLITE_NODE_ALTER_TABLE_STMT] = {range_meta_alter_table_stmt, 3},
    [SYNTAQLITE_NODE_TRANSACTION_STMT] = {NULL, 0},
    [SYNTAQLITE_NODE_SAVEPOINT_STMT] = {range_meta_savepoint_stmt, 1},
    [SYNTAQLITE_NODE_RESULT_COLUMN] = {range_meta_result_column, 2},
    [SYNTAQLITE_NODE_RESULT_COLUMN_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_SELECT_STMT] = {range_meta_select_stmt, 8},
    [SYNTAQLITE_NODE_ORDERING_TERM] = {range_meta_ordering_term, 1},
    [SYNTAQLITE_NODE_ORDER_BY_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_LIMIT_CLAUSE] = {range_meta_limit_clause, 2},
    [SYNTAQLITE_NODE_TABLE_REF] = {range_meta_table_ref, 3},
    [SYNTAQLITE_NODE_SUBQUERY_TABLE_SOURCE] = {range_meta_subquery_table_source, 2},
    [SYNTAQLITE_NODE_JOIN_CLAUSE] = {range_meta_join_clause, 4},
    [SYNTAQLITE_NODE_JOIN_PREFIX] = {range_meta_join_prefix, 1},
    [SYNTAQLITE_NODE_TRIGGER_EVENT] = {range_meta_trigger_event, 1},
    [SYNTAQLITE_NODE_TRIGGER_CMD_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_CREATE_TRIGGER_STMT] = {range_meta_create_trigger_stmt, 6},
    [SYNTAQLITE_NODE_CREATE_VIRTUAL_TABLE_STMT] = {range_meta_create_virtual_table_stmt, 4},
    [SYNTAQLITE_NODE_PRAGMA_STMT] = {range_meta_pragma_stmt, 3},
    [SYNTAQLITE_NODE_ANALYZE_STMT] = {range_meta_analyze_stmt, 2},
    [SYNTAQLITE_NODE_ATTACH_STMT] = {range_meta_attach_stmt, 3},
    [SYNTAQLITE_NODE_DETACH_STMT] = {range_meta_detach_stmt, 1},
    [SYNTAQLITE_NODE_VACUUM_STMT] = {range_meta_vacuum_stmt, 2},
    [SYNTAQLITE_NODE_EXPLAIN_STMT] = {range_meta_explain_stmt, 1},
    [SYNTAQLITE_NODE_CREATE_INDEX_STMT] = {range_meta_create_index_stmt, 5},
    [SYNTAQLITE_NODE_CREATE_VIEW_STMT] = {range_meta_create_view_stmt, 4},
    [SYNTAQLITE_NODE_VALUES_ROW_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_VALUES_CLAUSE] = {range_meta_values_clause, 1},
    [SYNTAQLITE_NODE_FRAME_BOUND] = {range_meta_frame_bound, 1},
    [SYNTAQLITE_NODE_FRAME_SPEC] = {range_meta_frame_spec, 2},
    [SYNTAQLITE_NODE_WINDOW_DEF] = {range_meta_window_def, 4},
    [SYNTAQLITE_NODE_WINDOW_DEF_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_NAMED_WINDOW_DEF] = {range_meta_named_window_def, 2},
    [SYNTAQLITE_NODE_NAMED_WINDOW_DEF_LIST] = {NULL, 0},
    [SYNTAQLITE_NODE_FILTER_OVER] = {range_meta_filter_over, 3},
};

#ifdef __cplusplus
}
#endif

#endif  // SYNQ_AST_BUILDER_H
