...
use crate::fields::{FieldDescriptor, FieldKind, FieldVal};
...
impl<'a> Node<'a> {
    ...
    /// Extract all fields into `buf`, returning the populated slice.
    pub fn fields<'b>(&self, source: &'a str, buf: &'b mut [FieldVal<'a>; MAX_FIELDS]) -> &'b [FieldVal<'a>] {
        let descriptors = FIELD_DESCRIPTORS[self.tag() as usize];
        let ptr = self.as_ptr();
        for (i, desc) in descriptors.iter().enumerate() {
            buf[i] = unsafe { desc.extract(ptr, source) };
        }
        &buf[..descriptors.len()]
    }
...
/// Maximum number of fields across all node types.
