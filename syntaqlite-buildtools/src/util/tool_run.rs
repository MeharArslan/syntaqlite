...
impl CArgs {
    ...
    pub(crate) const fn argv(&self) -> *const *const c_char {
        self.argv.as_ptr()
    }
}
...
pub(crate) fn prepare_c_args(program_name: &str, args: &[String]) -> CArgs {
    ...
    // Build pointer array: [program_name, arg1, arg2, ..., NULL]
    let mut ptrs = Vec::with_capacity(cstrings.len() + 1);
    ptrs.extend(cstrings.iter().map(|cs| cs.as_ptr()));
    ptrs.push(std::ptr::null()); // null terminator

    let count = c_int::try_from(ptrs.len() - 1).expect("argument count fits in c_int"); // Don't count the null terminator
