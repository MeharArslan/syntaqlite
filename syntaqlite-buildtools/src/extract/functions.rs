...
/// Generate the `SqliteFlag` enum mapping flag names to their bit indices.
pub(crate) fn generate_cflag_versions_rs(availability: &CflagAvailability) -> String {
    ...
    out.push_str("///\n");
    out.push_str("/// Pass a variant to [`SqliteFlags::has`](crate::util::SqliteFlags::has)\n");
    out.push_str("/// to test whether a feature is enabled on a grammar handle.\n");
    ...
    for entry in &availability.cflags {
        ...
        writeln!(out, "    {variant} = {index},").expect("write cannot fail");
    }

    out.push_str("}\n");
    out.push('\n');
    out.push_str("impl From<SqliteFlag> for u32 {\n");
    out.push_str("    fn from(flag: SqliteFlag) -> u32 {\n");
    out.push_str("        flag as u32\n");
    out.push_str("    }\n");
    out.push_str("}\n");
    out
}
...
/// Write the group-filtered Rust file to disk.
///
/// # Errors
///
/// Returns an error if creating the parent directory or writing the file fails.
...
pub(crate) fn write_cflag_versions_rs_for_group(
    ...
    let content = generate_cflag_versions_rs_for_group(availability, group);
    if let Some(parent) = output_path.parent() {
        fs::create_dir_all(parent).map_err(|e| format!("creating dir: {e}"))?;
    }
    fs::write(output_path, &content)
...
fn format_version_int(v: i32) -> String {
    ...
    let patch = v % 1_000;
    format!("{major}_{minor:03}_{patch:03}")
}
