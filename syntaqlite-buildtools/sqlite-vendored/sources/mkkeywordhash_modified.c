...
/* Number of keywords */
const int nKeyword = (sizeof(aKeywordTable)/sizeof(aKeywordTable[0]));
...
*/
static Keyword *findById(int id, Keyword *aKeywordTable, int nKeyword){
  int i;
...
*/
static void reorder(int *pFrom, Keyword *aKeywordTable, int nKeyword){
  int i = *pFrom - 1;
  int j;
  if( i<0 ) return;
...
** output.
*/
int mkkeyword_main(int argc, char **argv, Keyword *aKeywordTable, int nKeyword){
  int i, j, k, h;
  ...
  for(i=0; i<nKeyword; i++){
    Keyword *p = &aKeywordTable[i];
    if( p->substrId ){
      p->offset = findById(p->substrId, aKeywordTable, nKeyword)->offset + p->substrOffset;
    }
  }
