...
//! The two core dialect traits: [`GrammarNodeType`] and [`GrammarTokenType`].
//!
//! These are symmetric traits that dialect crates implement to plug into the
//! generic typed wrappers in `syntaqlite`:
//!
//! - [`GrammarNodeType`] — resolve a typed AST node from the parser arena.
//! - [`GrammarTokenType`] — resolve a typed token from a raw token code.
