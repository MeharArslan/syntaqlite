...
fn feed_tokens_select_1() {
    ...
    // Feed SELECT token.
    let r = session.feed(TokenType::Select, &source[0..6]).unwrap();
    assert!(r.is_none());

    // Feed integer literal token.
    let r = session.feed(TokenType::Integer, &source[7..8]).unwrap();
    assert!(r.is_none());
...
fn feed_tokens_with_semicolon() {
    ...
    session.feed(TokenType::Integer, &source[7..8]).unwrap();

    // SEMI is shifted but the ecmd reduction hasn't fired yet (needs lookahead).
    let r = session.feed(TokenType::Semi, &source[8..9]).unwrap();
    assert!(r.is_none());
...
fn feed_tokens_multi_statement() {
    ...
    let r = session.feed(TokenType::Select, &source[10..16]).unwrap();
    ...
    session.feed(TokenType::Integer, &source[17..18]).unwrap();
...
fn feed_token_skips_space() {
    ...
    let r = session.feed(TokenType::Space, &source[6..7]).unwrap();
    assert!(r.is_none());
...
fn feed_token_records_comment_trivia() {
    ...
    session.feed(TokenType::Comment, &source[7..15]).unwrap();
...
fn macro_straddle_rejected_by_parser() {
    ...
    session.feed(TokenType::Select, &source[0..6]).unwrap();
    session.feed(TokenType::Integer, &source[7..8]).unwrap();
    session.feed(TokenType::From, &source[9..13]).unwrap();

    // Macro: "foo!(x)" at offset 14, length 7.
    session.begin_macro(14, 7);
    session.feed(TokenType::Id, &source[19..20]).unwrap(); // "x"
    session.end_macro();

    // "y" outside macro — creates a straddling TableRef.
    session.feed(TokenType::Id, &source[22..23]).unwrap();
