...
/// A mask `#define` block mapping an SQLITE_OMIT_* flag to a bitmask value.
#[derive(Debug, Clone)]
pub struct MaskDefine {
    /// The mask symbol name, e.g. "RETURNING".
    pub name: String,
    /// The SQLITE_OMIT_* flag that disables it, e.g. "SQLITE_OMIT_RETURNING".
    ...
    while i < lines.len() {
        ...
        if let Some(rest) = trimmed.strip_prefix("#ifdef ") {
            ...
            if flag.starts_with("SQLITE_OMIT_") {
                // Look for the #else branch with the real bitmask value.
                if let Some(mask) = parse_ifdef_mask_block(&lines[i..], flag) {
                    masks.push(mask);
                }
            }
        }
...
fn parse_ifdef_mask_block(lines: &[&str], omit_flag: &str) -> Option<MaskDefine> {
