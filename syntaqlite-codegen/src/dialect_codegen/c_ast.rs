...
impl Display for CCodegenError {
...
impl std::error::Error for CCodegenError {}
...
pub fn generate_c_field_meta(items: &[Item], dialect: &str) -> String {
    try_generate_c_field_meta_typed(items, dialect).expect("c field metadata generation failed")
}

pub fn try_generate_c_field_meta(items: &[Item], dialect: &str) -> Result<String, String> {
    try_generate_c_field_meta_typed(items, dialect).map_err(|e| e.to_string())
}

pub fn try_generate_c_field_meta_typed(
    items: &[Item],
    dialect: &str,
) -> Result<String, CCodegenError> {
    let model = AstModel::new(items);
    try_generate_c_field_meta_from_model_typed(&model, dialect)
}

pub fn generate_c_field_meta_from_model(model: &AstModel<'_>, dialect: &str) -> String {
    try_generate_c_field_meta_from_model_typed(model, dialect)
        .expect("c field metadata generation failed")
}

pub fn try_generate_c_field_meta_from_model(
    model: &AstModel<'_>,
    dialect: &str,
) -> Result<String, String> {
    try_generate_c_field_meta_from_model_typed(model, dialect).map_err(|e| e.to_string())
}

pub fn try_generate_c_field_meta_from_model_typed(
    model: &AstModel<'_>,
    dialect: &str,
) -> Result<String, CCodegenError> {
    let enum_names = model.enum_names();
...
pub fn generate_c_fmt_arrays(items: &[Item]) -> String {
    try_generate_c_fmt_arrays_typed(items).expect("c fmt array generation failed")
}

pub fn try_generate_c_fmt_arrays(items: &[Item]) -> Result<String, String> {
    try_generate_c_fmt_arrays_typed(items).map_err(|e| e.to_string())
}

pub fn try_generate_c_fmt_arrays_typed(items: &[Item]) -> Result<String, CCodegenError> {
    let compiled =
        crate::fmt_compiler::try_compile_all(items).map_err(CCodegenError::FmtCompile)?;
    let model = AstModel::new(items);
...
fn c_field_kind(
    ...
    enum_names: &HashSet<&str>,
    flags_names: &HashSet<&str>,
) -> Result<&'static str, CCodegenError> {
    match field.storage {
        ...
        Storage::Inline => {
            ...
            if t == "SyntaqliteSourceSpan" {
                ...
                Err(CCodegenError::UnknownInlineType(t.clone()))
            }
        }
    }
}
