...
pub fn split_parse_c(parse_c: &str) -> Result<(String, String), String> {
    ...
    let main_body = c_transformer::CTransformer::new(parse_c)
        .remove_block_comment_containing("automatically generated by Lemon")
        .remove_text(&control_defines)
        .remove_text(&yytestcase)
        .replace_all(&parsing_tables_raw, "*/")  // keep `*/` to close preceding block comment
    ...
    // Build main engine file with SQLite blessing header.
    let main = {
        let mut w = c_writer::CWriter::new();
        w.sqlite_file_header();
        w.fragment(&main_body);
        w.finish()
    };

    // Strip the marker lines from parsing tables — they are comment delimiters
    // in the original parse.c context but bare code in the standalone data header.
    let parsing_tables = parsing_tables_raw
        .lines()
        .filter(|l| !l.contains("Begin parsing tables") && !l.contains("End of lemon-generated parsing tables"))
        .collect::<Vec<_>>()
        .join("\n");
