...
pub fn generate_rust_nodes(items: &[Item]) -> String {
    ...
    writeln!(out, "// @generated by syntaqlite-codegen — DO NOT EDIT").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "use syntaqlite_runtime::{{NodeId, NodeList, SourceSpan}};").unwrap();
...
pub fn generate_rust_ffi_nodes(items: &[Item]) -> String {
    ...
    writeln!(out, "#![allow(dead_code)]").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "use syntaqlite_runtime::{{NodeId, SourceSpan}};").unwrap();
    writeln!(out).unwrap();
...
pub fn generate_rust_ast(items: &[Item]) -> String {
    ...
    // Abstract types: type names used in index fields that aren't concrete nodes or lists.
    let abstract_names: HashSet<&str> = items.iter().flat_map(|i| {
        let fields = match i {
            Item::Node { fields, .. } => fields.as_slice(),
            _ => &[],
        };
        fields.iter().filter_map(|f| {
            if f.storage == Storage::Index {
                let t = f.type_name.as_str();
                if !node_names.contains(t) && !list_names.contains(t) {
                    return Some(t);
                }
            }
            None
    ...
    writeln!(out, "// @generated by syntaqlite-codegen — DO NOT EDIT").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "pub use syntaqlite_runtime::{{FromArena, NodeId, NodeReader, SourceSpan, Trivia, TriviaKind, TypedList}};").unwrap();
    writeln!(out, "pub(crate) use syntaqlite_runtime::NodeList;").unwrap();
    writeln!(out, "use std::marker::PhantomData;").unwrap();
    writeln!(out).unwrap();
    ...
    // Abstract type newtypes (Expr, Stmt, etc.)
    let mut sorted_abstract: Vec<&&str> = abstract_names.iter().collect();
    sorted_abstract.sort();
    for name in &sorted_abstract {
        writeln!(out, "/// An abstract `{}` node. Wraps `Node` for type clarity.", name).unwrap();
        writeln!(out, "#[derive(Debug, Clone, Copy)]").unwrap();
        writeln!(out, "pub struct {}<'a>(pub Node<'a>);", name).unwrap();
        writeln!(out).unwrap();
        writeln!(out, "impl<'a> std::ops::Deref for {}<'a> {{", name).unwrap();
        writeln!(out, "    type Target = Node<'a>;").unwrap();
        writeln!(out, "    fn deref(&self) -> &Node<'a> {{ &self.0 }}").unwrap();
        writeln!(out, "}}").unwrap();
        writeln!(out).unwrap();
        ...
        writeln!(out, "        Node::resolve(reader, id).map({})", name).unwrap();
        writeln!(out, "    }}").unwrap();
        writeln!(out, "}}").unwrap();
        writeln!(out).unwrap();
    }
