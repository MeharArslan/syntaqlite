...
pub fn generate_rust_dump(items: &[Item]) -> String {
    ...
    out.push_str("    let Some(node_ref) = session.node(id) else { return; };\n");
    out.push_str("    let source = session.source();\n");
    out.push_str("    let pad = \"  \".repeat(indent);\n\n");

    // List handling (before the match)
    out.push_str("    if node_ref.tag().is_list() {\n");
    out.push_str("        let list = node_ref.as_list().unwrap();\n");
    out.push_str("        let _ = writeln!(out, \"{pad}{:?} [{} items]\", node_ref.tag(), list.count);\n");
    ...
    // Match on tag
    out.push_str("    match node_ref.tag() {\n");
    ...
    for item in items {
        ...
        writeln!(out, "            let n = node_ref.{method}().unwrap();").unwrap();
    ...
    // Default case (Null + list tags already handled)
    out.push_str("        _ => {\n");
    out.push_str("            let _ = writeln!(out, \"{pad}{:?}\", node_ref.tag());\n");
    out.push_str("        }\n");
    out.push_str("    }\n");
    out.push_str("}\n");
