...
pub fn generate_rust_tokens(tokens: &[(String, u32)]) -> String {
    let mut out = String::new();
    writeln!(out, "{RUST_FILE_HEADER}").unwrap();
    writeln!(out).unwrap();
...
pub fn generate_rust_lib(dialect_fn: &str) -> String {
    ...
    writeln!(out, "/// Typed AST nodes for this dialect.").unwrap();
    writeln!(out, "///").unwrap();
    writeln!(out, "/// Each SQL statement type (e.g. `SELECT`, `INSERT`) has a corresponding struct").unwrap();
    writeln!(out, "/// with typed accessors for its fields. The top-level enum is [`ast::Stmt`],").unwrap();
    writeln!(out, "/// returned by [`StatementCursor::next_statement`] and").unwrap();
    writeln!(out, "/// [`LowLevelCursor::finish`](low_level::LowLevelCursor::finish).").unwrap();
    writeln!(out, "pub mod ast;").unwrap();
    writeln!(out, "mod wrappers;").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "use std::sync::LazyLock;").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "use syntaqlite_runtime::dialect::ffi as dialect_ffi;").unwrap();
    writeln!(out, "unsafe extern \"C\" {{").unwrap();
    writeln!(out, "    fn {dialect_fn}() -> *const dialect_ffi::Dialect;").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "static DIALECT: LazyLock<syntaqlite_runtime::Dialect<'static>> =").unwrap();
    writeln!(out, "    LazyLock::new(|| unsafe {{ syntaqlite_runtime::Dialect::from_raw({dialect_fn}()) }});").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "/// Low-level APIs for advanced use cases (e.g. custom token feeding/tokenizing).").unwrap();
    writeln!(out, "pub mod low_level {{").unwrap();
    writeln!(out, "    pub use crate::wrappers::{{LowLevelCursor, LowLevelParser, Tokenizer, TokenCursor}};").unwrap();
    writeln!(out, "    pub use crate::tokens::TokenType;").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Access the dialect handle (for use with `syntaqlite_runtime` APIs).").unwrap();
    writeln!(out, "    pub fn dialect() -> &'static syntaqlite_runtime::Dialect<'static> {{").unwrap();
    writeln!(out, "        &crate::DIALECT").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "pub use wrappers::{{Formatter, Parser, StatementCursor}};").unwrap();
    writeln!(out, "pub use syntaqlite_runtime::ParseError;").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "/// Configuration types for parsers and formatters.").unwrap();
    writeln!(out, "pub mod config {{").unwrap();
    writeln!(out, "    pub use syntaqlite_runtime::fmt::{{FormatConfig, KeywordCase}};").unwrap();
    writeln!(out, "    pub use syntaqlite_runtime::parser::ParserConfig;").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "mod tokens;").unwrap();
    out
}
...
pub fn generate_rust_wrappers() -> String {
    ...
    writeln!(out, "{RUST_FILE_HEADER}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "use std::ops::Range;").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "use crate::ast::{{FromArena, Stmt}};").unwrap();
    writeln!(out, "use crate::low_level::TokenType;").unwrap();
    writeln!(out, "use crate::ParseError;").unwrap();
    writeln!(out).unwrap();

    // Parser
    writeln!(out, "/// A parser pre-configured for this dialect.").unwrap();
    writeln!(out, "///").unwrap();
    writeln!(out, "/// Returns typed `StatementCursor` wrappers from `parse()`.").unwrap();
    writeln!(out, "pub struct Parser {{").unwrap();
    writeln!(out, "    inner: syntaqlite_runtime::Parser,").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "impl Parser {{").unwrap();
    writeln!(out, "    /// Create a new parser with default configuration.").unwrap();
    writeln!(out, "    pub fn new() -> Self {{").unwrap();
    writeln!(out, "        Parser {{").unwrap();
    writeln!(out, "            inner: syntaqlite_runtime::Parser::new(&crate::DIALECT),").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Create a parser with the given configuration.").unwrap();
    writeln!(out, "    pub fn with_config(config: &syntaqlite_runtime::parser::ParserConfig) -> Self {{").unwrap();
    writeln!(out, "        Parser {{").unwrap();
    writeln!(out, "            inner: syntaqlite_runtime::Parser::with_config(&crate::DIALECT, config),").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Access the current configuration.").unwrap();
    writeln!(out, "    pub fn config(&self) -> &syntaqlite_runtime::parser::ParserConfig {{").unwrap();
    writeln!(out, "        self.inner.config()").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Parse source text and return a `StatementCursor` for iterating statements.").unwrap();
    writeln!(out, "    pub fn parse<'a>(&'a mut self, source: &'a str) -> StatementCursor<'a> {{").unwrap();
    writeln!(out, "        StatementCursor {{ inner: self.inner.parse(source) }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();

    // StatementCursor
    writeln!(out, "/// A high-level parsing cursor with typed node access.").unwrap();
    writeln!(out, "pub struct StatementCursor<'a> {{").unwrap();
    writeln!(out, "    inner: syntaqlite_runtime::StatementCursor<'a>,").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "impl<'a> StatementCursor<'a> {{").unwrap();
    writeln!(out, "    /// Parse and return the next SQL statement as a typed `Stmt`.").unwrap();
    writeln!(out, "    ///").unwrap();
    writeln!(out, "    /// The returned `Stmt` borrows this cursor, so it cannot outlive it.").unwrap();
    writeln!(out, "    /// Returns `None` when all statements have been consumed.").unwrap();
    writeln!(out, "    pub fn next_statement(&mut self) -> Option<Result<Stmt<'_>, ParseError>> {{").unwrap();
    writeln!(out, "        let id = match self.inner.next_statement()? {{").unwrap();
    writeln!(out, "            Ok(id) => id,").unwrap();
    writeln!(out, "            Err(e) => return Some(Err(e)),").unwrap();
    writeln!(out, "        }};").unwrap();
    writeln!(out, "        let reader = self.inner.reader();").unwrap();
    writeln!(out, "        Some(Ok(Stmt::from_arena(reader, id).expect(\"parser returned invalid node\")))").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();

    // LowLevelParser
    writeln!(out, "/// A low-level parser for token-by-token feeding.").unwrap();
    writeln!(out, "///").unwrap();
    writeln!(out, "/// Feed tokens one at a time via `LowLevelCursor`.").unwrap();
    writeln!(out, "pub struct LowLevelParser {{").unwrap();
    writeln!(out, "    inner: syntaqlite_runtime::parser::LowLevelParser,").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "impl LowLevelParser {{").unwrap();
    writeln!(out, "    /// Create a new low-level parser with default configuration.").unwrap();
    writeln!(out, "    pub fn new() -> Self {{").unwrap();
    writeln!(out, "        LowLevelParser {{").unwrap();
    writeln!(out, "            inner: syntaqlite_runtime::parser::LowLevelParser::new(&crate::DIALECT),").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Create a low-level parser with the given configuration.").unwrap();
    writeln!(out, "    pub fn with_config(config: &syntaqlite_runtime::parser::ParserConfig) -> Self {{").unwrap();
    writeln!(out, "        LowLevelParser {{").unwrap();
    writeln!(out, "            inner: syntaqlite_runtime::parser::LowLevelParser::with_config(&crate::DIALECT, config),").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Bind source text and return a `LowLevelCursor` for token feeding.").unwrap();
    writeln!(out, "    pub fn feed<'a>(&'a mut self, source: &'a str) -> LowLevelCursor<'a> {{").unwrap();
    writeln!(out, "        LowLevelCursor {{ inner: self.inner.feed(source) }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();

    // LowLevelCursor
    writeln!(out, "/// A low-level cursor for feeding tokens one at a time.").unwrap();
    writeln!(out, "///").unwrap();
    writeln!(out, "/// After calling `finish()`, no further feeding methods may be called.").unwrap();
    writeln!(out, "pub struct LowLevelCursor<'a> {{").unwrap();
    writeln!(out, "    inner: syntaqlite_runtime::parser::LowLevelCursor<'a>,").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "impl<'a> LowLevelCursor<'a> {{").unwrap();
    writeln!(out, "    /// Feed a typed token to the parser.").unwrap();
    writeln!(out, "    ///").unwrap();
    writeln!(out, "    /// Returns `Ok(Some(stmt))` when a statement completes,").unwrap();
    writeln!(out, "    /// `Ok(None)` to keep going, or `Err` on parse error.").unwrap();
    writeln!(out, "    ///").unwrap();
    writeln!(out, "    /// The returned `Stmt` borrows this cursor, so it cannot be held").unwrap();
    writeln!(out, "    /// across further `feed_token` calls.").unwrap();
    writeln!(out, "    ///").unwrap();
    writeln!(out, "    /// `span` is a byte range into the source text bound by this cursor.").unwrap();
    writeln!(out, "    pub fn feed_token(").unwrap();
    writeln!(out, "        &mut self,").unwrap();
    writeln!(out, "        token_type: TokenType,").unwrap();
    writeln!(out, "        span: Range<usize>,").unwrap();
    writeln!(out, "    ) -> Result<Option<Stmt<'_>>, ParseError> {{").unwrap();
    writeln!(out, "        match self.inner.feed_token(token_type.into(), span)? {{").unwrap();
    writeln!(out, "            None => Ok(None),").unwrap();
    writeln!(out, "            Some(id) => {{").unwrap();
    writeln!(out, "                let reader = self.inner.base().reader();").unwrap();
    writeln!(out, "                Ok(Some(Stmt::from_arena(reader, id).expect(\"parser returned invalid node\")))").unwrap();
    writeln!(out, "            }}").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Signal end of input.").unwrap();
    writeln!(out, "    ///").unwrap();
    writeln!(out, "    /// Returns `Ok(Some(stmt))` if a final statement completed,").unwrap();
    writeln!(out, "    /// `Ok(None)` if there was nothing pending, or `Err` on parse error.").unwrap();
    writeln!(out, "    ///").unwrap();
    writeln!(out, "    /// After calling `finish()`, no further feeding methods may be called.").unwrap();
    writeln!(out, "    pub fn finish(&mut self) -> Result<Option<Stmt<'_>>, ParseError> {{").unwrap();
    writeln!(out, "        match self.inner.finish()? {{").unwrap();
    writeln!(out, "            None => Ok(None),").unwrap();
    writeln!(out, "            Some(id) => {{").unwrap();
    writeln!(out, "                let reader = self.inner.base().reader();").unwrap();
    writeln!(out, "                Ok(Some(Stmt::from_arena(reader, id).expect(\"parser returned invalid node\")))").unwrap();
    ...
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Mark subsequent fed tokens as being inside a macro expansion.").unwrap();
    writeln!(out, "    pub fn begin_macro(&mut self, call_offset: u32, call_length: u32) {{").unwrap();
    writeln!(out, "        self.inner.begin_macro(call_offset, call_length)").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// End the innermost macro expansion region.").unwrap();
    writeln!(out, "    pub fn end_macro(&mut self) {{").unwrap();
    writeln!(out, "        self.inner.end_macro()").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();

    // Formatter
    writeln!(out, "/// SQL formatter pre-configured for this dialect.").unwrap();
    writeln!(out, "pub struct Formatter {{").unwrap();
    writeln!(out, "    inner: syntaqlite_runtime::fmt::Formatter<'static>,").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "impl Formatter {{").unwrap();
    writeln!(out, "    /// Create a formatter with default configuration.").unwrap();
    writeln!(out, "    pub fn new() -> Result<Self, &'static str> {{").unwrap();
    writeln!(out, "        let inner = syntaqlite_runtime::fmt::Formatter::new(&crate::DIALECT)?;").unwrap();
    writeln!(out, "        Ok(Formatter {{ inner }})").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Create a formatter with the given configuration.").unwrap();
    writeln!(out, "    pub fn with_config(config: crate::config::FormatConfig) -> Result<Self, &'static str> {{").unwrap();
    writeln!(out, "        let inner = syntaqlite_runtime::fmt::Formatter::with_config(&crate::DIALECT, config)?;").unwrap();
    writeln!(out, "        Ok(Formatter {{ inner }})").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Access the current configuration.").unwrap();
    writeln!(out, "    pub fn config(&self) -> &crate::config::FormatConfig {{").unwrap();
    writeln!(out, "        self.inner.config()").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Format SQL source text.").unwrap();
    writeln!(out, "    pub fn format(").unwrap();
    writeln!(out, "        &mut self,").unwrap();
    writeln!(out, "        source: &str,").unwrap();
    writeln!(out, "    ) -> Result<String, ParseError> {{").unwrap();
    writeln!(out, "        self.inner.format(source)").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();

    // Tokenizer
    writeln!(out, "/// A tokenizer for SQL.").unwrap();
    writeln!(out, "pub struct Tokenizer {{").unwrap();
    writeln!(out, "    inner: syntaqlite_runtime::parser::Tokenizer,").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "impl Tokenizer {{").unwrap();
    writeln!(out, "    /// Create a new tokenizer.").unwrap();
    writeln!(out, "    pub fn new() -> Self {{").unwrap();
    writeln!(out, "        Tokenizer {{").unwrap();
    writeln!(out, "            inner: syntaqlite_runtime::parser::Tokenizer::new(),").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Bind source text and return a cursor for iterating typed tokens.").unwrap();
    writeln!(out, "    pub fn tokenize<'a>(&'a mut self, source: &'a str) -> TokenCursor<'a> {{").unwrap();
    writeln!(out, "        TokenCursor {{").unwrap();
    writeln!(out, "            inner: self.inner.tokenize(source),").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Zero-copy variant: bind a null-terminated source and return a").unwrap();
    writeln!(out, "    /// `TokenCursor`. The source must be valid UTF-8 (panics otherwise).").unwrap();
    writeln!(out, "    pub fn tokenize_cstr<'a>(&'a mut self, source: &'a std::ffi::CStr) -> TokenCursor<'a> {{").unwrap();
    writeln!(out, "        TokenCursor {{").unwrap();
    writeln!(out, "            inner: self.inner.tokenize_cstr(source),").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    ...
    // TokenCursor
    writeln!(out, "/// An active tokenizer session yielding typed tokens.").unwrap();
    writeln!(out, "pub struct TokenCursor<'a> {{").unwrap();
    writeln!(out, "    inner: syntaqlite_runtime::parser::TokenCursor<'a>,").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "impl<'a> Iterator for TokenCursor<'a> {{").unwrap();
    writeln!(out, "    type Item = (TokenType, &'a str);").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    fn next(&mut self) -> Option<Self::Item> {{").unwrap();
    writeln!(out, "        let raw = self.inner.next()?;").unwrap();
    writeln!(out, "        let tt = TokenType::from_raw(raw.token_type)").unwrap();
    writeln!(out, "            .unwrap_or(TokenType::Illegal);").unwrap();
    writeln!(out, "        Some((tt, raw.text))").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out, "}}").unwrap();
