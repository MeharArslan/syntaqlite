...
pub fn generate_rust_nodes(items: &[Item]) -> String {
    ...
    writeln!(out, "// @generated by syntaqlite-codegen — DO NOT EDIT").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "use crate::fields::{{FieldDescriptor, FieldKind, FieldVal}};").unwrap();
    writeln!(out, "use crate::nodes::{{NodeList, SourceSpan}};").unwrap();
    ...
    writeln!(out, "    /// Extract all fields into `buf`, returning the populated slice.").unwrap();
    writeln!(out, "    pub fn fields<'b>(&self, source: &'a str, buf: &'b mut [FieldVal<'a>; MAX_FIELDS]) -> &'b [FieldVal<'a>] {{").unwrap();
    writeln!(out, "        let descriptors = FIELD_DESCRIPTORS[self.tag() as usize];").unwrap();
    writeln!(out, "        let ptr = self.as_ptr();").unwrap();
    writeln!(out, "        for (i, desc) in descriptors.iter().enumerate() {{").unwrap();
    writeln!(out, "            buf[i] = unsafe {{ desc.extract(ptr, source) }};").unwrap();
    writeln!(out, "        }}").unwrap();
    writeln!(out, "        &buf[..descriptors.len()]").unwrap();
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    ...
    // MAX_FIELDS const
    let max_fields = items.iter().filter_map(|item| {
        if let Item::Node { fields, .. } = item { Some(fields.len()) } else { None }
    }).max().unwrap_or(0);
    writeln!(out, "/// Maximum number of fields across all node types.").unwrap();
    writeln!(out, "pub const MAX_FIELDS: usize = {};", max_fields).unwrap();
    writeln!(out).unwrap();
...
/// Generate Rust source for a `dump_node` function that recursively prints
/// the AST in an indented text format.
///
/// Static helpers (`dump_child`, `dump_span`) live in hand-written
/// `crate::dump`; this only generates the per-node dispatch.
pub fn generate_rust_dump(items: &[Item]) -> String {
    let enum_names: HashSet<&str> = items.iter().filter_map(Item::as_enum_name).collect();
    let flags_names: HashSet<&str> = items.iter().filter_map(Item::as_flags_name).collect();
    let mut out = String::new();

    out.push_str("// @generated by syntaqlite-codegen — DO NOT EDIT\n\n");
    out.push_str("use std::fmt::Write;\n");
    out.push_str("use crate::nodes::NULL_NODE;\n");
    out.push_str("use crate::dump::{dump_child, dump_span};\n");
    out.push_str("use crate::Session;\n");
    out.push_str("use super::nodes::*;\n\n");

    // Main function
    out.push_str("/// Dump an AST node tree as indented text.\n");
    out.push_str("pub fn dump_node(session: &Session<'_>, id: u32, out: &mut String, indent: usize) {\n");
    out.push_str("    if id == NULL_NODE { return; }\n");
    out.push_str("    let Some(node) = session.node(id) else { return; };\n");
    out.push_str("    let source = session.source();\n");
    out.push_str("    let pad = \"  \".repeat(indent);\n\n");
    ...
    for item in items {
        match item {
            Item::Node { name, fields, .. } => {
                writeln!(out, "        Node::{name}(n) => {{").unwrap();
                writeln!(out, "            let _ = writeln!(out, \"{{pad}}{name}\");").unwrap();

                for field in fields {
                    let fname = rust_field_name(&field.name);
                    match field.storage {
                        Storage::Index => {
                            writeln!(out, "            dump_child(session, &pad, \"{}\", n.{fname}, out, indent);",
                                field.name).unwrap();
                        }
                        Storage::Inline => {
                            if field.type_name == "SyntaqliteSourceSpan" {
                                writeln!(out, "            dump_span(&pad, \"{}\", &n.{fname}, source, out);",
                                    field.name).unwrap();
                            } else if flags_names.contains(field.type_name.as_str()) {
                                writeln!(out, "            let _ = writeln!(out, \"{{pad}}  {}: {{}}\", n.{fname}.dump_str());",
                                    field.name).unwrap();
                            } else if enum_names.contains(field.type_name.as_str()) {
                                writeln!(out, "            let _ = writeln!(out, \"{{pad}}  {}: {{}}\", n.{fname}.as_str());",
                                    field.name).unwrap();
                            } else {
                                writeln!(out, "            let _ = writeln!(out, \"{{pad}}  {}: {{:?}}\", n.{fname});",
                                    field.name).unwrap();
                            }
                        }
                    }
            ...
            Item::List { name, .. } => {
                writeln!(out, "        Node::{name}(list) => {{").unwrap();
                writeln!(out, "            let _ = writeln!(out, \"{{pad}}{name} [{{}} items]\", list.count);").unwrap();
                out.push_str("            for &child_id in list.children() {\n");
                out.push_str("                dump_node(session, child_id, out, indent + 1);\n");
                out.push_str("            }\n");
                out.push_str("        }\n");
            }
            _ => {}
...
/// Map a field to its FieldKind string.
fn field_kind_str(
    field: &Field,
    enum_names: &HashSet<&str>,
    flags_names: &HashSet<&str>,
    ...
    match field.storage {
        ...
        Storage::Inline => {
            if field.type_name == "SyntaqliteSourceSpan" {
                ...
                panic!("unknown inline type for field codegen: {}", field.type_name)
            }
        }
    }
}
...
fn generate_field_dispatch_table(out: &mut String, items: &[Item]) {
    writeln!(out, "const FIELD_DESCRIPTORS: &[&[FieldDescriptor]] = &[").unwrap();
