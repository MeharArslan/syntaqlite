...
### 4.4 Keyword version + cflag gating (NOT YET IMPLEMENTED)

The keyword table (generated by `mkkeywordhash`) will gain parallel arrays for version and cflag gating:
...
The keyword lookup function will check both version and cflags, respecting polarity:
...
| `sqlite_keyword.c`          | `aKWSince[]` + `aKWCFlag[]` + `aKWCFlagPolarity[]` arrays + checks in lookup                             | TODO      |
...
## 5. Phase 3: Oracle Tests (IN PROGRESS)
...
### 5.3 Integration tests (COMPLETED — tokenizer level)

Test file: `syntaqlite/tests/multiversion.rs`

Tests use the runtime `Tokenizer` API directly (with `set_dialect_config`) and raw `u32` token type values (since `TokenType::from_raw` is `pub(crate)`).
...
3. **Oracle tests (tokenizer level)** — COMPLETED. `syntaqlite/tests/multiversion.rs` with 11 green tests (TK_PTR, TK_QNUMBER, parse-level, baseline) + 7 ignored red tests (keyword gating). All behavior verified against real sqlite3 shells compiled from official amalgamations.

4. **Keyword `since` + cflag arrays** — TODO. Add `aKWSince[]`, `aKWCFlag[]`, and `aKWCFlagPolarity[]` to the `mkkeywordhash` codegen pipeline. Add version + cflag checks to keyword lookup using `SYNQ_VER_LT` and `SYNQ_HAS_CFLAG` macros. Un-ignore the 7 keyword tests in `multiversion.rs` and verify they pass.

5. **Subquery flag** — TODO. Add `sawSubquery` field to parse context. Set it in the subquery grammar actions. Check after parsing when `SQLITE_OMIT_SUBQUERY` is active.

6. **CFlag oracle tests** — TODO. Add cflag-gated tests to `multiversion.rs` (OMIT_WINDOWFUNC, OMIT_SUBQUERY, etc.).
