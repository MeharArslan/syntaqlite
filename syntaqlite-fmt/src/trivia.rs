...
use syntaqlite_parser::{Trivia, TriviaKind};

use crate::doc::{DocArena, DocId, NIL_DOC};
...
impl<'a> TriviaCtx<'a> {
    ...
    pub fn drain_before(&self, before: u32, arena: &mut DocArena<'a>) -> DrainResult {
            ...
            match t.kind {
                TriviaKind::LineComment => {
                    if has_newline {
                        ...
                        // Always emit a hardline before the comment to ensure
                        // it appears on a new line (the ops may not have a
                        // Line between the previous content and this point).
        ...
        // HardLine already makes fits() return false for the enclosing
        // group, so no explicit BreakParent is needed for leading trivia.
