use syntaqlite_fmt::{render, DocArena, FormatConfig, KeywordCase, NIL_DOC};
...
fn long_select_breaks_with_indentation() {
    let mut arena = DocArena::new();
    // SELECT nest(4, line cols) line FROM " " table
    // The line break after SELECT is inside the nest so cols get indented.
    let select = arena.keyword("SELECT");
    let sp1 = arena.line();
    let cols = comma_list(
        &mut arena,
...
fn nested_groups_subquery_in_parens() {
    ...
    // Build: SELECT * FROM (SELECT a, b FROM t) AS sub
    let select = arena.keyword("SELECT");
    let sp = arena.text(" ");
    let star = arena.text("*");
    ...
    // Subquery in parens: group( nest(4, "(" softline sub_group) softline ")" )
    // softline before ")" is outside nest so it gets the outer indent.
    let lp = arena.text("(");
    ...
    // Wide: everything fits one line
    assert_eq!(
        render(&arena, doc, &default_config()),
        "SELECT * FROM (SELECT a, b FROM t) AS sub"
    );

    // Width 24: outer breaks, paren_group breaks (flat=20 > 24-5=19),
    // but sub_group fits flat (18 <= 24-4=20)
